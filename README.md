...
- **Hardening** — rate limiting, helmet security headers, strict input validation, non-root Docker runtime.
...
| Tests      | Jest (unit, >85% coverage enforced)     |
...
| GET | `/health` | Liveness + database connectivity (used by the Docker healthcheck) |
...
├── common/      # shared DTOs (pagination) and decorators
...
├── health/      # liveness endpoint for orchestration
...
├── tmdb/        # typed TMDB API client (all TMDB knowledge isolated here)
...
The suite covers services, controllers, guards, the TMDB client, sync logic, cache behaviour and edge cases (invalid input, missing resources, duplicate watchlist entries, revoked refresh tokens). Current coverage is ~98% statements.
